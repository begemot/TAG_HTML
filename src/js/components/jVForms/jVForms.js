    ...
    var CLASS_SUBMIT = 'vf-submit',
        ...
        config = {
           ...
           pattern: {
               ...
               stringRu: '^[а-яёА-ЯЁ]+$',        // '^[^a-zA-Z]+$' сторка любая не содержащая US букв.
               ...
               phoneDash: '^\\+\\d\\s\\([\\d]{3}\\)\\s[\\d]{3}\\s-\\s[\\d]{2}\\s-\\s[\\d]{2}$', // номер в формате (999) 999-9999|(999) 999 9999. /// +9 (999) 999 - 99 - 99
               phoneAlong: '^\\([\\d]{3}\\)\\s[\\d]{7}$',                        // номер в формате (999) 9999999. 
               
               test: '^([а-яёА-ЯЁ]+|[а-яёА-ЯЁ]+\\s[а-яёА-ЯЁ]+|[а-яёА-ЯЁ]+\\s[а-яёА-ЯЁ]+\\s[а-яёА-ЯЁ]+)$'
               // test: '^test$' 
           },
    ...
    var notification = {
        ...
        notice: {
            ...
            bgColor: {
                ...
                hex: ['#fc5c65','#30d230'],
                rgb: ['rgba(252,92,101,1)','rgba(48,210,48,.7)']
            },
            ...
            valid: function( el ) {
                ...
                // ie<9 not rgba
                try {
                    el.style.borderColor = (config.mode === 1) ? notification.notice.bgColor.rgb[1] : 
                                                                     notification.notice.bgColor.hex[2];
                } catch(exp) {
                    el.style.borderColor = (config.mode === 1) ? notification.notice.bgColor.hex[1] : 
                                                                     notification.notice.bgColor.hex[2];
                }
            ...
            invalid: function( el ) {
                ...
                // ie<9 not rgba
                try {
                    el.style.borderColor = notification.notice.bgColor.rgb[0];
                } catch(exp) {
                    el.style.borderColor = notification.notice.bgColor.hex[0];
                }
            ...
            getStyle: function( el ) {
                ...
                notification.notice.bgColor.hex.push(computedStyle.borderColor);
            }
...
jVForms.libClass = {
     ...
     */
     contains: function( el, nameClass ) {
         ...
         var classListArray = classList.toString().split( space );
